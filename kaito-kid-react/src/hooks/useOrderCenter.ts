...
type ReturnCenterMap = Record<number, OrderReturnCenterDTO>;

function toReturnCenterMap(items: OrderReturnCenterDTO[]): ReturnCenterMap {
  return Object.fromEntries(items.map((item) => [item.orderId, item]));
}
...
export function useOrderCenter() {
  ...
  const [returnCenter, setReturnCenter] = useState<ReturnCenterMap>({});
  ...
  const loadReturnCenter = useCallback(async () => {
    ...
    setReturnCenter(toReturnCenterMap(result.data));
    return true;
  }, []);
  ...
  const loadOrders = useCallback(async (silent = false) => {
    ...
    const [orderResult, returnResult] = await Promise.all([
      customerOrderApi.getMyOrders(),
      orderReturnApi.getAll(),
    ]);

    if (orderResult.success && orderResult.data) {
      setOrders([...orderResult.data].sort(
        (x, y) => new Date(y.createdAt).getTime() - new Date(x.createdAt).getTime(),
      ));
    } else if (!silent) {
      setOrders([]);
      toast.error(orderResult.error || 'Không tải được đơn hàng');
    }

    if (returnResult.success && returnResult.data) {
      setReturnCenter(toReturnCenterMap(returnResult.data));
    }
  ...
  const counts = useMemo(() => {
    const result: Record<OrderStatusFilterValue, number> = {
      all: orders.length,
      pending: 0,
      shipping: 0,
      completed: 0,
    ...
    orders.forEach((order) => {
      const group = customerOrderStatusGroup(order.status);
      if (group !== 'all') result[group] += 1;
    });
    return result;
  }, [orders]);
  ...
  const matching = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      return [order.orderCode, order.customerName, order.trackingCode, ...order.items.map((item) => item.productName)]
  ...
  const pendingReviewCount = useMemo(
    () => orders
      ...
      .reduce((sum, order) => sum + pendingReviewItems(order).length, 0),
    [orders],
  );
  const activeOrderCount = counts.pending + counts.shipping;
  const activeReturnCount = useMemo(
    () => Object.values(returnCenter)
      .filter((item) => item.request && !['rejected', 'refunded'].includes(item.request.status)).length,
    [returnCenter],
  );

  const openTracking = async (order: CustomerOrderDTO) => {
    setTrackingCode(order.orderCode || String(order.id));
    ...
    if (result.success && result.data) {
      ...
      toast.error(result.error || 'Chưa lấy được hành trình đơn hàng');
      setTrackingCode('');
    }
  ...
  const reorder = async (orderId: number) => {
    ...
    if (!result.success || !result.data) {
      toast.error(result.error || 'Không thể mua lại đơn này');
      return;
    }
    await refreshCart();
  ...
  const startReview = (order: CustomerOrderDTO, item?: CustomerOrderItemDTO) => {
    ...
    if (!target) {
      toast.success('Bạn đã đánh giá tất cả sản phẩm trong đơn này.');
      return;
    }
    setSelected(null);
    setReviewingItem({ order, item: target });
  };

  const reviewSubmitted = (orderId: number, productId: number) => {
    setOrders((current) => current.map((order) => order.id !== orderId ? order : {
      ...order,
      items: order.items.map((item) => item.productId === productId
  ...
  const openReturn = async (order: CustomerOrderDTO) => {
    setSelected(null);
    setReturnChecking(true);

    // Luôn kiểm tra lại ngay lúc khách bấm để tránh dùng metadata cũ hoặc coi lỗi API là "không đủ điều kiện".
    const result = await orderReturnApi.getAll();
    setReturnChecking(false);

    if (!result.success || !result.data) {
      toast.error(result.error
        ? `Không kết nối được dịch vụ đổi/trả: ${result.error}`
        : 'Không kết nối được dịch vụ đổi/trả. Vui lòng thử lại sau.');
      return;
    }

    const latest = toReturnCenterMap(result.data);
    setReturnCenter(latest);
    const info = latest[order.id];

    if (!info) {
      toast.error('Không lấy được thông tin đổi/trả của đơn hàng này. Vui lòng làm mới trang rồi thử lại.');
      return;
    }
  ...
  const submitReturn = async () => {
    ...
    if (returnReason === 'other' && returnNote.trim().length < 5) {
      toast.error('Vui lòng mô tả rõ lý do đổi/trả');
      return;
    }

    setReturnSubmitting(true);
    const result = await orderReturnApi.create(returningOrder.id, {
      reason: returnReason,
    ...
    if (!result.success || !result.data) {
      toast.error(result.error || 'Không thể gửi yêu cầu đổi/trả');
      return;
    }

    setReturnCenter((current) => ({
      ...current,
      [returningOrder.id]: {
        ...(current[returningOrder.id] || { orderId: returningOrder.id, eligible: false }),
        eligible: false,
        request: result.data,
      },
    }));
  ...
  const cancelReturn = async (requestId: number) => {
    ...
    if (!result.success) {
      toast.error(result.error || 'Không thể rút yêu cầu');
      return;
    }
    toast.success(result.data?.message || 'Đã rút yêu cầu đổi/trả');
    await loadReturnCenter();
